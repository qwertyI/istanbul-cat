...
function createHandler(opts) {
    ...
    app.use('/:commit/asset', express.static(ASSETS_DIR));
    app.use('/:commit/asset', express.static(path.join(ASSETS_DIR, 'vendor')));
    ...
    app.get('/:commit', function(req, res, next) {
        commit = req.params.commit
        core.setGlobalCoverage(commit)
        var origUrl = url.parse(req.originalUrl).pathname,
            origLength = origUrl.length;
        if (origUrl.charAt(origLength - 1) !== '/') {
            origUrl += '/';
    ...
    app.get('/:commit/show', function (req, res) {
        var origUrl = url.parse(req.originalUrl).pathname,
            u = url.parse(req.url).pathname,
            pos = origUrl.indexOf(u),
            file = req.query.p;
            commit_id = req.params.commit
        if (pos >= 0) {
        ...
        core.render(file, res, origUrl + '/' + commit_id);
    });

    //reset coverage to baseline on POST /reset
    app.post('/:commit/reset', function (req, res) {
